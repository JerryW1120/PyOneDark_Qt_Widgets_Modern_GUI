...
class player(QMainWindow):
    ...
    def open(self):
        ...
        if not self._mime_types:
            ...
            elif MOV not in self._mime_types:
                self._mime_types.append(MOV)
            elif MPX not in self._mime_types:
                self._mime_types.append(MPX)


        file_dialog.setMimeTypeFilters(self._mime_types)

        default_mimetype = MP4 if is_windows else MOV
        if default_mimetype in self._mime_types:
            file_dialog.selectMimeTypeFilter(default_mimetype)

        movies_location = QStandardPaths.writableLocation(QStandardPaths.MoviesLocation) # 路径在这里，嵌进去的时候在这里修改
